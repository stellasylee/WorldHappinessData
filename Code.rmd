...
GDPMap09 <- ggplot() + geom_polygon(data=map09, 
  ...
  scale_fill_gradientn(name= "GDP per capita", colours = rainbow(7))+
  labs(title = "GDP (2009)")

lifeMap09 <- ggplot() + geom_polygon(data=map09, 
          aes(x=long, y=lat, group=group, fill = lifeExpect), 
          color="white", size = 0.2) + 
  scale_fill_gradientn(name= "Life expectancy at birth", colours = rainbow(7))+
  labs(title = "Life Expectancy (2009)")

corrupMap09 <- ggplot() + geom_polygon(data=map09, 
          aes(x=long, y=lat, group=group, fill = corruption), 
          color="white", size = 0.2) + 
  scale_fill_gradientn(name = "corruption", colours = rainbow(7)) +
  labs(title= "Corruption (2009)")  
happyMap09 <- ggplot() + geom_polygon(data=map09, 
          aes(x=long, y=lat, group=group, fill = HPI), 
          color="white", size = 0.2) + 
  scale_fill_gradientn(name = "HPI", colours = rainbow(7)) +
  labs(title= "HPI (2009)")
...
maps09 <- subplot(
  ...
  titleY = FALSE, titleX = FALSE, margin = 0.5
) 
...
p <- plot_geo(map09, lat = ~lat, lon = ~long) %>%
  add_trace(
    z = ~HPI, text = ~paste(country), 
    locations = g, locationmode = 'country names'
  )%>%
  add_markers(
    text = ~paste(country, paste("HPI:", HPI), sep = "<br />"),
    color = ~HPI, symbol = I("square"), size = I(8), hoverinfo = "text"
  ) %>%
  colorbar(title = "Happy Planet Index") %>%
  layout(
    title = 'Happy Planet Index (2009)', geo = g
  )
p
```
...
ggplot(data=final09)+  geom_point(aes(x=GDP, y=HPI, colour = region))
ggplot(data=final09)+  geom_point(aes(x=corruption, y=HPI, colour = region))

ggplot(data=final12)+  geom_point(aes(x=lifeExpect, y=HPI, colour = region))
ggplot(data=final12)+  geom_point(aes(x=GDP, y=HPI, colour = region))
ggplot(data=final12)+  geom_point(aes(x=corruption, y=HPI, colour = region))

ggplot(data=final16)+  geom_point(aes(x=lifeExpect, y=HPI, colour = region))
ggplot(data=final16)+  geom_point(aes(x=GDP, y=HPI, colour = region))
ggplot(data=final16)+  geom_point(aes(x=corruption, y=HPI, colour = region))
...
scatterplot3d(final09$GDP, final09$lifeExpect, final09$HPI, color = region, type = "h", main="3D Scatter Plot", xlab = "GDP", ylab = "Life Expectancy", zlab = "Happiness")
```
